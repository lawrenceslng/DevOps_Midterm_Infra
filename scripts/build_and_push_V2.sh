...
docker build -t devops-midterm-frontend:"$TAG" -f "$APP_REPO_PATH/client/Dockerfile" "$APP_REPO_PATH"
# Build the backend Docker image
# docker build --target=production -t devops-midterm-backend:latest -t devops-midterm-backend:"$TAG" "$APP_REPO_PATH/server"
docker build --target=production -t devops-midterm-backend:"$TAG" "$APP_REPO_PATH/server"
